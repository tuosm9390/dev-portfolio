...
vi.mock("@/lib/geul/firebase", () => ({
  getAllowedGeulAuthorEmail: () => "tuosm123@gmail.com",
  getGeulAuth: () => null,
  getGeulFirestore: () => null,
}));
...
describe("GeulEditor", () => {
  it("Firebase 설정이 없을 때 설정 안내와 작성 레이아웃을 렌더링한다", () => {
    render(<GeulEditor />);

    expect(screen.getByRole("heading", { level: 1, name: "geul" })).toBeInTheDocument();
    expect(screen.getByText("Firebase 설정이 필요합니다.")).toBeInTheDocument();
