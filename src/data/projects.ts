...
const rawProjects: Project[] = [
  {
    ...
    description: `# Synapso.dev 기술 설명서
...
`,
  ...
  },
  {
    ...
    description: `# Cafe Book 프로젝트 설명서

## 프로젝트 개요

Cafe Book은 카페 지도 탐색, 카페 평가와 즐겨찾기, 커피 레시피 기록과 공유를 하나의 SPA로 묶은 React 기반 웹 애플리케이션입니다. Firebase Auth와 Firestore를 백엔드로 사용하고, Kakao Maps SDK와 Kakao REST API를 통해 지도, 위치, 카페 이미지 탐색 흐름을 구성합니다.

이 프로젝트의 핵심 가치는 실제 서버 애플리케이션을 별도로 두지 않고도 인증, 권한, 데이터 저장, 지도 UX, PWA 배포, E2E 검증까지 프론트엔드 중심으로 운영 가능하게 만든 점입니다.

## 핵심 사용자 흐름

\`\`\`text
사용자
  ├─ 홈(\`/\`)에서 지도, 레시피, 커뮤니티로 이동
  ├─ 지도(\`/map\`)에서 카페 검색, 현재 위치 이동, 카페 카드 확인
  ├─ 로그인 후 레시피(\`/recipe\`) 작성, 수정, 삭제
  ├─ 레시피 상세(\`/recipe/:id\`)에서 공개 레시피 조회, 복사, 가이드 실행
  ├─ 커뮤니티(\`/community\`)에서 공개 레시피 피드 탐색
  └─ 관리자(\`/admin\`)에서 카페 CRUD, 이미지 캐시 초기화, 집계 필드 마이그레이션
\`\`\`

## 아키텍처

Cafe Book은 Vite가 번들링하는 클라이언트 사이드 React 애플리케이션입니다. \`src/main.tsx\`가 React 앱을 부트스트랩하고, \`src/App.tsx\`가 \`BrowserRouter\`와 라우트 보호 컴포넌트를 조합합니다.

\`\`\`text
cafe-book/
├── src/
│   ├── api/                  # Firebase, Kakao, E2E mock 데이터 접근 계층
│   ├── components/           # 공용 UI, 지도/카페 카드, 라우트 가드
│   ├── components/recipe/    # 레시피 폼, 카드, 타임라인, 가이드, 소셜 UI
│   ├── hooks/                # 인증, 카페, 레시피, 위치, 이미지 로딩 상태 관리
│   ├── pages/                # 라우트 단위 화면
│   ├── types/                # Cafe, User, Recipe 등 도메인 타입
│   ├── utils/                # 검색, 레시피 계산, Kakao SDK 로딩, 에러 변환
│   └── test/                 # Vitest 전역 설정과 일부 단위 테스트
├── tests/
│   ├── e2e/                  # Playwright 사용자 흐름 테스트
│   └── unit/                 # Vitest 단위/렌더 테스트
├── firebase.json             # Firebase Hosting, Firestore database 설정
├── firestore.rules           # Firestore 권한 정책
├── vite.config.ts            # React, PWA, Vitest 설정
└── playwright.config.ts      # E2E 서버와 브라우저 프로젝트 설정
\`\`\`

## 데이터와 권한 모델

Firestore 컬렉션은 코드와 보안 규칙 기준으로 \`cafes\`, \`interactions\`, \`users\`, \`recipes\`, \`recipeLikes\`, \`recipeComments\`를 사용합니다.

| 도메인 | 주요 파일 | 권한 모델 |
| --- | --- | --- |
| 카페 | \`src/api/cafeApi.ts\` | 누구나 읽기 가능, 생성/삭제는 관리자만 가능, 이미지와 집계 필드는 제한적 업데이트 허용 |
| 카페 상호작용 | \`src/api/interactionApi.ts\` | 누구나 읽기 가능, 인증 사용자는 자신의 리뷰와 즐겨찾기만 쓰기 가능 |
| 레시피 | \`src/api/recipeApi.ts\` | 공개 레시피는 누구나 읽기 가능, 비공개와 쓰기 작업은 작성자 중심 |
| 레시피 소셜 | \`src/api/recipeInteractionApi.ts\` | 좋아요와 댓글은 인증 사용자 중심, 공개 레시피 댓글은 공개 조회 가능 |
| 관리자 | \`src/hooks/useAdminStatus.ts\`, \`firestore.rules\` | \`users/{uid}.role == admin\` 또는 \`admin@cafebook.com\` 이메일 기준 |

Firebase 초기화는 \`src/api/firebase.ts\`에서 수행합니다. Firestore는 네트워크 차단 환경 대응을 위해 \`experimentalForceLongPolling\`을 사용하고, 반복 실패 시 \`terminate\`와 IndexedDB persistence 초기화를 통해 자가 치유를 시도합니다.

## 지도와 위치 흐름

지도 화면은 \`src/pages/MapPage.tsx\`가 상태를 조합하고 \`src/components/MapContainer.tsx\`가 Kakao Map 렌더링을 담당합니다. Kakao SDK 로딩은 \`src/utils/mapLoader.ts\`에서 분리되어 있고, E2E 환경에서는 실제 지도 SDK 대신 테스트 가능한 대체 DOM을 렌더링합니다.

현재 위치 흐름은 \`src/hooks/useGeolocation.ts\`에서 권한 상태와 좌표 요청을 관리합니다. \`MapPage\`는 위치 요청 성공 시 선택된 카페를 해제하고 지도 중심을 즉시 갱신합니다. 모바일에서는 하단 Archives 패널 상태에 따라 현재 위치 버튼, 권한 안내, 선택 카페 카드의 위치를 조절합니다.

## 레시피 도메인

레시피는 \`src/types/recipe.ts\`의 \`Recipe\`와 \`ExtractionStep\` 타입을 중심으로 구현되어 있습니다. 단순 텍스트 기록이 아니라 원두명, 원산지, 로스팅 포인트, 물 온도, 원두량, 물량, 추출 비율, 단계별 물 사용량, 누적 물량, 단계 시간, 향미 프로필과 태그를 구조화해 저장합니다.

레시피 작성과 수정은 \`src/components/recipe/RecipeForm.tsx\`를 공유합니다. 상세 화면은 \`src/pages/RecipeDetailPage.tsx\`에서 소유자 권한, 공개 여부, 복사, 삭제, 타임라인, 추출 가이드, 향미 시각화를 함께 처리합니다. 과거 데이터의 단계 시간이 누락된 경우 \`src/pages/recipeDetailTiming.tsx\`에서 상세 화면 진입 시점에 계산해 표시하는 lazy normalization 방식을 사용합니다.

## 테스트 전략

테스트는 Vitest와 Playwright를 함께 사용합니다.

| 계층 | 명령 | 범위 |
| --- | --- | --- |
| 타입과 번들 | \`npm run build\` | \`tsc\`와 Vite production build |
| 단위 테스트 | \`npm run test:unit\` | 유틸, 훅, 인증, 레시피 계산, 소셜 로직 |
| E2E 테스트 | \`npm run test:e2e\` | 로그인, 관리자 CRUD, 지도, 레시피 작성/상세, 공개 접근 |

E2E에서는 \`window.isE2ETest\` 플래그를 사용합니다. API 계층은 개발 모드와 이 플래그를 확인해 Firestore 대신 \`localStorage\`의 \`mock_user\`, \`mock_cafes\`, \`mock_recipes\`, \`mock_interaction_*\` 데이터를 사용합니다. 이 방식은 실제 Firebase 데이터를 오염시키지 않으면서 라우팅과 UI 흐름을 브라우저에서 검증하기 위한 프로젝트 고유 패턴입니다.

## PWA와 배포

\`vite-plugin-pwa\`가 \`vite.config.ts\`에서 설정되어 있으며, 앱 이름, 아이콘, \`standalone\` display, portrait orientation, Workbox runtime caching을 구성합니다. Firebase Hosting은 \`firebase.json\`에서 \`dist\`를 public directory로 사용하고, SPA 직접 경로 새로고침을 위해 모든 경로를 \`/index.html\`로 rewrite합니다. \`public/_redirects\`도 동일하게 \`/* /index.html 200\` fallback을 제공합니다.

## 기술 스택

| 영역 | 사용 기술 |
| --- | --- |
| 앱 프레임워크 | React 18, TypeScript 5, Vite 5 |
| 라우팅 | React Router DOM v7 |
| 스타일링 | Tailwind CSS, Radix UI primitives, class-variance-authority, tailwind-merge |
| 지도 | Kakao Maps JavaScript SDK, \`react-kakao-maps-sdk\`, Kakao REST Image Search |
| 백엔드 | Firebase Auth, Firestore, Firebase Hosting |
| PWA | \`vite-plugin-pwa\`, Workbox runtime caching |
| 테스트 | Vitest, Testing Library, Playwright |

## 구현상의 특징

- Firebase API 접근을 \`src/api\`에 집중시켜 화면은 대부분 훅과 API 함수에 의존합니다.
- 지도 화면은 PC와 모바일의 레이아웃 요구가 달라 \`Layout\`, \`Sidebar\`, \`MapPage\`, \`MapContainer\`, \`CafeInfoPanel\`이 역할을 나눕니다.
- E2E mock은 테스트 파일이 아니라 런타임 API 계층에서 분기하므로 실제 사용자 흐름과 가까운 방식으로 검증합니다.
- Firestore rules는 클라이언트 권한 모델과 맞물려 공개 읽기, 소유자 쓰기, 관리자 전용 작업을 분리합니다.
- 레시피 상세는 공개 접근과 로그인 사용자 기능이 공존하므로 \`ProtectedRoute\`가 아닌 상세 내부 권한 검사를 사용합니다.
`,
    techStack: [
      "React",
      "TypeScript",
