...
- Modified principles: 
    - Principle I: Toss-Style Modern UX/UI (디자인 감성 및 인터랙션 정의)
    - Principle II: Atomic UI System (shadcn/ui 기반 컴포넌트 체계)
    - Principle III: Data-Driven Centralization (데이터 관리 전략)
    - Principle IV: Strict Validation & Type Safety (Zod 및 TS 타입 안전성)
    - Principle V: Performance & Accessibility First (최적화 및 접근성)
- Added sections: 기술적 제약 및 보안 요구사항, 개발 워크플로우 및 품질 게이트
- Templates status: 
...
### IV. Strict Validation & Type Safety
모든 사용자 입력과 외부 데이터 연동 시 Zod를 활용한 정교한 유효성 검사를 수행한다. TypeScript의 `any` 타입 사용을 절대 금지하며, 인터페이스와 타입을 명확히 정의하여 런타임 에러를 사전에 방지한다.
...
## 개발 워크플로우 및 품질 게이트
모든 기능 구현은 `.specify/templates`의 계획서와 체크리스트를 기반으로 수행한다. 새로운 기능을 추가할 때는 해당 기능의 목적과 아키텍처를 문서화해야 하며, 기존 UI 패턴과의 일관성을 유지하는지 검증하는 단계를 포함한다.
